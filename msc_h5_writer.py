"""
MSC Nastran HDF5 Writer Module
===============================
pyNastran OP2 modellerini MSC Nastran native HDF5 (.h5) formatina yazar.

Bu modul hem op2_to_h5.py (CLI) hem de op2_to_h5_gui.py (GUI) tarafindan
kullanilir. Tum HDF5 yazim fonksiyonlari burada tanimlidir.
"""
...
def _write_input_grids(h5, op2, _log):
    ...
    if nodes is None:
        _log('    [UYARI] INPUT/NODE/GRID: nodes verisi bulunamadi (OP2 geometri icermiyor olabilir)')
        return
    ...
    if isinstance(nodes, dict) and len(nodes) > 0:
        ...
        _log('    [UYARI] INPUT/NODE/GRID: nodes bos veya beklenmeyen format')
...
def _write_input_elements(h5, op2, _log):
    ...
    if not elements:
        _log('    [UYARI] INPUT/ELEMENT: elements verisi bulunamadi')
        return
    ...
    for etype, elems in sorted(by_type.items()):
        ...
        if etype == 'CQUAD4':
            ...
            _log(f'    [UYARI] INPUT/ELEMENT/{etype}: desteklenmeyen eleman tipi, atlaniyor')
            continue
...
def _write_input_properties(h5, op2, _log):
    ...
    if not properties:
        _log('    [UYARI] INPUT/PROPERTY: properties verisi bulunamadi')
        return
    ...
    for ptype, props in sorted(by_type.items()):
        ...
        if ptype == 'PROD':
            ...
            _log(f'    [UYARI] INPUT/PROPERTY/{ptype}: desteklenmeyen ozellik tipi, atlaniyor')
            continue
...
def _write_input_materials(h5, op2, _log):
    ...
    if not materials:
        _log('    [UYARI] INPUT/MATERIAL: materials verisi bulunamadi')
        return
    ...
    for mtype, mats in sorted(by_type.items()):
        ...
        if mtype == 'MAT1':
            ...
            _log(f'    [UYARI] INPUT/MATERIAL/{mtype}: desteklenmeyen malzeme tipi, atlaniyor')
            continue
...
def _write_input_rigid_elements(h5, op2, _log):
    ...
    if not rigid:
        _log('    [UYARI] INPUT/ELEMENT/RBE: rigid_elements verisi bulunamadi')
        return
...
def _write_input_loads(h5, op2, _log):
    ...
    if not loads:
        _log('    [UYARI] INPUT/LOAD: loads verisi bulunamadi')
        return
...
def _write_input_coords(h5, op2, _log):
    ...
    if not coords:
        _log('    [UYARI] INPUT/COORDINATE_SYSTEM: coords verisi bulunamadi')
        return
...
def _write_input_constraints(h5, op2, _log):
    ...
    if spc_rows:
        ...
        _log('    [UYARI] INPUT/CONSTRAINT: spcs verisi bulunamadi')
...
def _write_input_parameters(h5, op2, _log):
    ...
    if not params:
        _log('    [UYARI] INPUT/PARAMETER: params verisi bulunamadi')
        return
